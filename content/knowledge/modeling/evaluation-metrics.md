...
$$
\text{WOE}_i = \ln\!\left(\frac{\text{坏客户占比}_i}{\text{好客户占比}_i}\right) = \ln\!\left(\frac{B_i / B_{\text{total}}}{G_i / G_{\text{total}}}\right)
$$
...
$$
\text{IV} = \sum_{i=1}^{n} \left(\frac{B_i}{B_{\text{total}}} - \frac{G_i}{G_{\text{total}}}\right) \times \text{WOE}_i
$$

IV 是 WOE 的加权求和：分箱中好坏客户差距越大（括号越大），WOE 绝对值越大，对 IV 的贡献越多。
...
$$
\text{KS} = \max_{\text{threshold}} \left| F_{\text{bad}}(x) - F_{\text{good}}(x) \right|
$$

其中 $F_{\text{bad}}(x)$ 是坏客户按评分排序的累积分布函数，$F_{\text{good}}(x)$ 是好客户的对应分布。
...
def calc_ks(y_true, y_pred_proba):
    """y_true: 真实标签（1=坏客户）; y_pred_proba: 模型预测的违约概率"""
    bad_scores  = y_pred_proba[y_true == 1]
    good_scores = y_pred_proba[y_true == 0]
    ks_stat, p_value = ks_2samp(bad_scores, good_scores)
    return ks_stat

# 或通过分档计算（更直观，可见每档的好坏分布）
...
$$
\text{TPR（召回率）} = \frac{TP}{TP + FN} \quad \text{（坏客户被正确识别的比例）}
$$

$$
\text{FPR（误拒率）} = \frac{FP}{FP + TN} \quad \text{（好客户被误拒的比例）}
$$
...
$$
\text{AUC} = P(\text{模型给随机一个坏客户的评分} > \text{给随机一个好客户的评分})
$$
...
$$
\text{KS} \approx 2 \times (\text{AUC} - 0.5)
$$
...
其中：
- $E_i$：建模时（训练/验证集）第 $i$ 分段的客户占比（**预期分布**）
...
def calc_psi(expected_scores, actual_scores, n_bins=10):
    """
    expected_scores: 建模时的评分序列（numpy array）
    actual_scores:   上线后某月的评分序列
    """
    # 用建模期的分位数定义分箱边界
    ...
    def get_bin_counts(scores):
        counts = np.histogram(scores, bins=breakpoints)[0]
        rates  = counts / len(scores)
        rates  = np.where(rates == 0, 0.0001, rates)  # 防止 ln(0)
        return rates
    
    expected_rates = get_bin_counts(expected_scores)
    actual_rates   = get_bin_counts(actual_scores)
...
for month in production_months:
    ...
    print(f"{month}: PSI = {psi:.4f} {'⚠️ 偏移' if psi > 0.1 else '✓ 稳定'}")
```
...
$$
\text{CSI} = \sum_{i=1}^{n} \left(A_i - E_i\right) \times \ln\!\left(\frac{A_i}{E_i}\right)
$$

公式与 PSI 相同，只是对象从评分变为单个特征变量。
...
其中 $R^2_j$ 是将第 $j$ 个特征作为因变量、其余特征作为自变量进行回归时的决定系数。
...
# 使用 WOE 编码后的特征矩阵
vif_df = calc_vif(X_woe_encoded)

# 逐步剔除 VIF > 10 的特征（每次只删 VIF 最高的，然后重新计算）
while vif_df['VIF'].max() > 10:
    worst_feature = vif_df.iloc[0]['feature']
    print(f"删除高共线性特征：{worst_feature}（VIF={vif_df.iloc[0]['VIF']:.1f}）")
    X_woe_encoded = X_woe_encoded.drop(columns=[worst_feature])
    vif_df = calc_vif(X_woe_encoded)
```
