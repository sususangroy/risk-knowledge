...
以评分阈值为横轴，累积百分比（0–1）为纵轴，图上有两条曲线：

- **F_bad(x)（坏客户累积曲线）**：坏客户集中于高风险评分区，曲线在横轴左侧就迅速攀升至高位
- **F_good(x)（好客户累积曲线）**：好客户分散在低风险区，曲线上升平缓，始终低于坏客户曲线

两条曲线在某处的垂直距离最大，该最大值即为 KS（即下图红色竖线 D 的长度），对应的阈值就是好坏分离效果最佳的切点。

![KS 曲线示意图](/images/ks-curve.png)

**两个极端**：

- 两条曲线完全重叠 → KS = 0，模型没有任何区分能力
- 两条曲线完全分离 → KS = 1，模型完美区分好坏客户

实际中好的信贷模型 KS 在 0.3–0.5 之间，意味着在最优切点处，坏客户的累积比例比好客户高出 30–50 个百分点。
